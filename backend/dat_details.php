...
if(isset($_GET['id'])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT preservation_signature, original_signature, brand, brand_of_box, samplerate, notes FROM dat WHERE dpo_id = :dpo_id");
        ...
        if($result) {
            ...
            echo "<p>Sample Rate: " . htmlspecialchars($result['samplerate']) . "</p>";
            echo "<p>Notes: " . htmlspecialchars($result['notes']) . "</p>";
        } else {
            echo "DAT details not found.";
        }
